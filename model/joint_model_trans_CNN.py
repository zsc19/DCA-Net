...
class Joint_model(nn.Module):
    def __init__(self, _, hidden_dim, batch_size, max_length, n_class, n_tag, embedding_matrix):
        ...
        self.CNN = CNN(input_size=300, hidden_dim=300, dropout_rate=config.emb_dorpout)
...
class CNN(nn.Module):
    def __init__(self, input_size=64, hidden_dim=256, dropout_rate=0.2):
        super().__init__()
        self.conv1 = nn.Conv1d(in_channels=input_size, out_channels=hidden_dim, kernel_size=1, padding=0)
        self.conv3 = nn.Conv1d(in_channels=input_size, out_channels=hidden_dim, kernel_size=3, padding=1)
        self.conv5 = nn.Conv1d(in_channels=input_size, out_channels=hidden_dim, kernel_size=5, padding=2)
        self.droput_out = nn.Dropout(dropout_rate)
        self.linear = nn.Linear(hidden_dim * 3, input_size)
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 1)
        # x [B, D(C)=64, N]
        x1 = self.droput_out(self.conv1(x))
        x3 = self.droput_out(self.conv3(x))
        x5 = self.droput_out(self.conv5(x))

        # cat [B, D(C)=256 * 3, N]
        out = self.linear(torch.cat([x1, x3, x5], dim=1).permute(0, 2, 1))
        # out [B, N, D(C)]
        return self.relu(out)


# model = CNN(64, 256, 0.2)
#
# inputs = torch.rand(4 * 10 * 64).reshape(4, 10, 64)
# output = model(inputs)
